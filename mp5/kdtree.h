...
class KDTree
{
  ...
  private:
     ...
     */
    double getDistance(const Point<Dim>& target1,
                    const Point<Dim>& target2) const;
    Point<Dim> quickSelect(int left, int right, int dimension, int midIndex);
    int partition(int left, int right, int dimension, int pivotIndex);
    void buildTree(int left, int right, int dimension);
    void buildTreeHelper(int left, int right, int dimension, KDTreeNode *&node);
    void _copy(const KDTree &other);
    void _destroy();
    Point<Dim> helperNN(const Point<Dim>& query, int start, int end, int dimension) const;
