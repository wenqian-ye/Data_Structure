...
size_t search_idx(const std::vector<T>& elements, const C& val, size_t l, size_t h){
    ...
    if(elements[mid] > val){
        return search_idx(elements, val, l, mid);
    }
    if(val > elements[mid] && val < elements[mid+1])
        return mid + 1;
    else{
        return search_idx(elements, val, mid, h);
    }
}
...
size_t insertion_idx(const std  ::vector<T>& elements, const C& val)
{
    ...
    size_t low = 0;
    size_t high = elements.size();
    if(high == 0)
        return 0;
    if(elements[0] > val)
        return 0;
    if(elements[high-1] < val)
        return high;
    if(elements[high-1] == val)
        return high - 1;
    return search_idx(elements, val, low, high-1);
